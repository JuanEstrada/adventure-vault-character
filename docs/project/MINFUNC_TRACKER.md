...
| S23 | Audit mixed-source precedence gaps | Identify the remaining compendium/base/imported-content precedence permutations that still need coverage. | S18 | Done | Audit complete: current tests already cover XML-base single-import, XML-base multi-import, import-collision, duplicate, title-collision, and fallback-base merge paths. Smallest remaining gap is fallback JSON base with multiple imported packs plus explicit source-policy note assertions for mixed-source collision permutations.
| S24 | Add mixed-source regression tests | Add targeted regressions for the remaining precedence permutations and source-merge behaviors. | S23 | Done | Added fallback JSON base coverage for multiple imported packs, including catalog source-policy note assertions and pack-state continuity across mixed-source collisions. |
| S25 | Verify parity for new hardening cases | Confirm repository/migration parity for any newly added validation slices. | S22, S24 | Done | Fallback JSON mixed-source cases now reload cleanly from the database with the same pack-state set and catalog source-policy notes. |
| S26 | Close Phase 5 hardening slice | Update tracker/docs once the remaining validation gaps are proven closed or explicitly deferred. | S25 | Done | Phase 5 mixed-source hardening is closed; docs now reflect the fallback JSON regression and parity coverage. |
