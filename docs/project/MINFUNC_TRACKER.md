...
| Core mutations persist correctly after reopen | Yes | Met | Widget coverage now proves direct spell-slot spend/restore, class-resource persistence, and baseline inventory mutations remain correct after navigating back and reopening the same character. |
...
| Targeted automated coverage exists for the release-bar flows | Yes | Met | Coverage now includes create/edit/reopen, rests, combat, direct spell-slot actions, class-resource reopen checks, and minimum inventory smoke/regression behavior for the MinFunc path. |
...
| S17  | Verify persistence and reopen           | Confirm or fix reopen behavior for minimum session changes.                              | S15, S16     | Done          | Verified spells, inventory, class resources, rests, and character reopen paths; no functional reopen regression was found, so this slice closed with coverage/docs only. Full `test/widget_test.dart` still exposes unrelated existing spell-selection expectation failures in `generated ability set updates when class changes` and `warlock uses known-spell mode with pact slot progression`, which should be triaged outside this reopen slice. |
